...
module GA
  ...
  class Optimizer
    ...
    def initialize(args = {})
      ...
      unless @cfg[:p_mutation] >= 0 and @cfg[:p_mutation] <= 1 then
        raise "Error with the assigned mutation probability:\n it is #{@cfg[:p_mutation]} but must be 0 <= p_mutation <= 1 "
      end
      
      unless @cfg[:p_crossover] >= 0 and @cfg[:p_crossover] <= 1 then
        raise "Error with the assigned crossover probability:\n it is #{@cfg[:p_crossover]} but must be 0 <= p_crossover <= 1 " 
      end
      
      @cfg.merge! args
      @nbit = @cfg[:nbit] #@pop.max_bit # is the number f bits required to encode into a binary string the chromosome
      @population = Population.new( @cfg[:ncr] , @cfg[:i_o])
      @start_points = []
      @status = :filling
      @iteration = 0
      @best = []
    ...
    def loop( ary = nil )
      ...
      until converged? or @iteration > @cfg[:npop]
        ...
        # @population.size is used to set the number of 
        # chromosomes in the new generation
        childs = evolve( bit_selected, @population.size, @cfg[:p_crossover], @cfg[:p_mutation] ) 
        # child is converted into an array of hashes, each with keys:
        # :chromosome, :bitstring , :fitness
        ...
        puts "Maximum number of iteration reached: #{@cfg[:npop]}" if @iteration == @cfg[:npop] ##########
        puts "_________________________________________________________"
        
        # these lines ar used to do a convergence plot, 
        # i.e. all the fitnesses for the current population
        if @cfg[:pconv] == true
    ...
    def converged?
      if @iteration >= 3
        xx = []
        3.times{ |t| xx << ( @best[-(t+1)][:fitness]).round(3) }
        # for me @best[0][:fitness].round(3) doesn't work
        
        if xx[0] == xx[1] && xx[1] == xx[2] && xx[2] == xx[0]
          puts "Converged, the last three generations are identical."
          puts "The best chromosome is #{@best[-1].inspect}"
          true
        else
          false
        end # if a
    ...
    def selection(pop)
      ...
      pop.size.times do
        ...
        # if unfortunatly j = i, evaluates j again
        j = rand( pop.size ) while j == i 
        sel << (pop[i][:fitness] < pop[j][:fitness] ?  pop[i] : pop[j])
      end
      return sel
    end
    
    # the chromosome is a string of '0' and '1', rate [0,1]. 
    # mutant is also a string
    ...
    def mutation( bitstring , rate )
      ...
      bitstring.size.times do |i|
        gene = bitstring[i]
        # change the bit value only if the rand [0,1] 
        # is minor than the mutation probability
        mutant << ((rand < rate) ? ((gene == '1') ? "0" : "1") : gene)
      end # chromosome
      return mutant
    end # def mutation
    
    # both father and mather are strings, rate [0,1] is 
    ...
    def crossover( father, mother, rate )
      ...
      # don't do the cross over if rand is maior or equal than
      # the crossover probability 
      ...
      point = 0
      # sets the crossover point randomly
      point = rand(mother.size) while point == 0 or point == mother.size 
      return father[0..point-1] + mother[point..(mother.size)] , mother[0..point-1] + father[point..(father.size)] 
    end
    
    # this is used to convert the input values into a binary 
    # string (the chromosome)
    # Input: array of floats. Output: one bit string
    def encode( ary = [] )
      ary_b = ""
      ary.each do |v|
        i_b =  v.to_i.abs.to_bin( @nbit )
        # how to encode the sign: the first bit is 0 if 
        # i_b is >= 0, and 1 if it's < 0
    ...
    # this is used to decode the binary chromosome string 
    ...
    def decode( str )
      ...
      # splits the string: 
      # eg."000111110011" -> ["","000","","111","","110","","011"]
      str_a = str.split( rexp )  
      str_a = str_a.delete_if{ |v| v == ""}  # ["000", "111", "110", "011"]
      str_a.size.times do |i| 
        if i <= str_a.size-2
          # the first bit of the element with an odd index 
          # is the sign of the floating number
          str_a[ 2*i ][0..0] == "0" ? sign = "+" : sign = "-"
          # this returns: [ +00.7 , +2.6 ], each element 
          # in the array is in decimal codification
